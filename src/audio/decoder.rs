/* --- LOONIX-TUNES src/audio/decoder.rs --- */
...
pub fn spawn_decoder(path: String, producer: HeapProd<f32>, control: Arc<DecoderControl>) {
    ...
    loop {
        ...
        loop {
            ...
            if control.is_seeking.load(Ordering::SeqCst)
                && control.seeking_state.load(Ordering::SeqCst) != SEEK_STATE_READY
            {
                continue;
            }

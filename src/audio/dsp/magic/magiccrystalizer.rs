/* --- LOONIX-TUNES src/audio/dsp/magic/magiccrystalizer.rs --- */
...
pub fn get_crystal_amount_arc() -> &'static AtomicU32 {
    CRYSTAL_AMOUNT.get_or_init(|| AtomicU32::new(0.20_f32.to_bits()))
}
...
impl DspProcessor for Crystalizer {
    fn process(&mut self, input: &[f32], output: &mut [f32]) {
        ...
        let len = input.len();
        let drive = 3.0;
        let alpha = self.current_freq / (self.current_freq + 48000.0);

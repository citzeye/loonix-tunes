...
pub struct StdABRepeat {
    state: StdABRepeatState,
...
impl StdABRepeat {
    ...
    pub fn reset(&mut self) {
        self.state = StdABRepeatState::Off;
        self.point_a = 0.0;
        self.point_b = 0.0;
    }
    ...
    pub fn check_loop(&self, current_position: f64) -> Option<f64> {
        if self.state == StdABRepeatState::Active && current_position >= self.point_b {
            Some(self.point_a)
        } else {
            None
        }
    ...
    pub fn state(&self) -> StdABRepeatState {
        self.state
    }

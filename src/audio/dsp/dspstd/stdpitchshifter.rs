...
unsafe impl Send for StdPitchShifter {}
unsafe impl Sync for StdPitchShifter {}

impl StdPitchShifter {
    pub fn new() -> Self {
        let options =
...
impl Drop for StdPitchShifter {
    fn drop(&mut self) {
        unsafe { rubberband_delete(self.handle) };
    }
}

impl DspProcessor for StdPitchShifter {
    fn process(&mut self, input: &[f32], output: &mut [f32]) {
        ...
        unsafe {
            rubberband_process(self.handle, in_ptrs.as_ptr(), frames as i32, false);
            ...
            if avail > 0 {
                ...
                rubberband_retrieve(self.handle, self.l_out_ptr.as_mut_ptr(), avail as i32);
